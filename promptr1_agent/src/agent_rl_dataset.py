...
class ToolRLDataset(RLHFDataset):
    ...
    def __init__(
        ...
        if self.use_default_tool_template and self.env is not None:  #True
    ...
    def __getitem__(self, item):
        ...
        # replacement = "# Tools\n\n你可以调用一个或多个函数来帮助完成用户的请求。\n\n以下提供了可用函数的签名，包含在 <tools></tools> 标签中"
        # if target in raw_prompt:
        #     raw_prompt = raw_prompt.replace(target, replacement)
