...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if "em" in batch.non_tensor_batch:
        em = batch.non_tensor_batch["em"]
        metrics.update(
            {
                # answer score
                "critic/em/mean": np.mean(em),
                "critic/em/max": np.max(em),
                "critic/em/min": np.min(em),
            }
        )
    
    if "f1" in batch.non_tensor_batch:
        f1 = batch.non_tensor_batch["f1"]
        metrics.update(
            {
                # answer score
                "critic/f1/mean": np.mean(f1),
                "critic/f1/max": np.max(f1),
                "critic/f1/min": np.min(f1),
            }
        )
        
    # if "sim" in batch.non_tensor_batch:
    #     sim = batch.non_tensor_batch["sim"]
    #     metrics.update(
    #         {
    #             # answer score
    #             "critic/sim/mean": np.mean(sim),
    #             "critic/sim/max": np.max(sim),
    #             "critic/sim/min": np.min(sim),
    #         }
    #     )
    
    if "sm" in batch.non_tensor_batch:
        sm = batch.non_tensor_batch["sm"]
        metrics.update(
            {
                # answer score
                "critic/sim/mean": np.mean(sm),
                "critic/sim/max": np.max(sm),
                "critic/sim/min": np.min(sm),
            }
        )
